...
- [X] T049 [P] Add accessibility verification covering active-actor selection, project and task creation, assignment and reassignment, board navigation, task-detail opening, comment submission, notification access, and keyboard task movement. Verify visible focus and announcement of the task title, destination column, and success or safe failure result in tests/e2e/kanban-accessibility.spec.ts and docs/accessibility.md.
- [X] T050 [P] Add cross-cutting integration coverage that: (a) loads the seeded three-project workspace and its task locations within the QC-004 two-second threshold; (b) propagates accepted board and recipient-notification updates to connected clients within the QC-006 two-second threshold; (c) exercises simultaneous updates to the same task and verifies one succeeds while stale versions are rejected without partial changes; (d) covers every documented input boundary and invalid relationship; and (e) verifies diagnostics contain a correlation ID but no raw request bodies, credentials, or sensitive values; in tests/integration/performance-boundary.test.ts, tests/integration/input-validation.test.ts, and tests/integration/diagnostics.test.ts.
